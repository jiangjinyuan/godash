...
func testSendOutputs(r *Harness, t *testing.T) {
	genSpend := func(amt godashutil.Amount) *chainhash.Hash {
	...
	// input.
	txid := genSpend(godashutil.Amount(5 * godashutil.SatoshiPerBitcoin))
	...
	txid = genSpend(godashutil.Amount(500 * godashutil.SatoshiPerBitcoin))
	blockHashes, err = r.Node.Generate(1)
	if err != nil {
		t.Fatalf("unable to generate single block: %v", err)
	}
...
func testGenerateAndSubmitBlock(r *Harness, t *testing.T) {
	...
	output := wire.NewTxOut(godashutil.SatoshiPerBitcoin, pkScript)

	const numTxns = 5
	txns := make([]*godashutil.Tx, 0, numTxns)
	for i := 0; i < numTxns; i++ {
		...
		txns = append(txns, godashutil.NewTx(tx))
	}
...
func testMemWalletReorg(r *Harness, t *testing.T) {
	...
	expectedBalance := godashutil.Amount(250 * godashutil.SatoshiPerBitcoin)
	...
	// chain.
	expectedBalance = godashutil.Amount(0)
	walletBalance = harness.ConfirmedBalance()
	if expectedBalance != walletBalance {
		t.Fatalf("wallet balance incorrect: expected %v, got %v",
			expectedBalance, walletBalance)
...
func testMemWalletLockedOutputs(r *Harness, t *testing.T) {
	...
	outputAmt := godashutil.Amount(50 * godashutil.SatoshiPerBitcoin)
...
func TestHarness(t *testing.T) {
	...
	// outputs.
	expectedBalance := godashutil.Amount(numMatureOutputs * 50 * godashutil.SatoshiPerBitcoin)
