...
type NotificationHandlers struct {
	...
	OnRecvTx func(transaction *godashutil.Tx, details *btcjson.BlockDetails)
	...
	//
	// NOTE: Deprecated. Use OnRelevantTxAccepted instead.
	OnRedeemingTx func(transaction *godashutil.Tx, details *btcjson.BlockDetails)
	...
	// made to register for the notification and the function is non-nil.
	OnTxAccepted func(hash *chainhash.Hash, amount godashutil.Amount)
	...
	// such as btcwallet.
	OnAccountBalance func(account string, balance godashutil.Amount, confirmed bool)
...
func parseFilteredBlockConnectedParams(params []json.RawMessage) (int32,
	*wire.BlockHeader, []*godashutil.Tx, error) {
	...
	transactions := make([]*godashutil.Tx, len(hexTransactions))
	for i, hexTx := range hexTransactions {
		...
		transactions[i], err = godashutil.NewTxFromBytes(transaction)
		if err != nil {
			return 0, nil, nil, err
		}
	}
...
// notifications.
func parseChainTxNtfnParams(params []json.RawMessage) (*godashutil.Tx,
	*btcjson.BlockDetails, error) {
	...
	return godashutil.NewTx(&msgTx), block, nil
}
...
func parseAccountBalanceNtfnParams(params []json.RawMessage) (account string,
	balance godashutil.Amount, confirmed bool, err error) {
	...
	// Bounds check amount.
	bal, err := godashutil.NewAmount(fbal)
	if err != nil {
		return "", 0, false, err
	}
...
//
// NOTE: Deprecated. Use LoadTxFilterAsync instead.
func (c *Client) NotifyReceivedAsync(addresses []godashutil.Address) FutureNotifyReceivedResult {
	// Not supported in HTTP POST mode.
	if c.config.HTTPPostMode {
		return newFutureError(ErrWebsocketsRequired)
	}
...
//
// NOTE: Deprecated. Use LoadTxFilter instead.
func (c *Client) NotifyReceived(addresses []godashutil.Address) error {
...
func (c *Client) RescanEndBlockAsync(startBlock *chainhash.Hash,
	addresses []godashutil.Address, outpoints []*wire.OutPoint,
...
// NOTE: This is a btcd extension ported from github.com/decred/dcrrpcclient
// and requires a websocket connection.
func (c *Client) LoadTxFilterAsync(reload bool, addresses []godashutil.Address,
