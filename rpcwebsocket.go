...
//
// NOTE: This extension was ported from github.com/decred/dcrd
func (f *wsClientFilter) addAddress(a godashutil.Address) {
	switch a := a.(type) {
...
func (f *wsClientFilter) addAddressStr(s string, params *chaincfg.Params) {
	...
	// script.
	a, err := godashutil.DecodeAddress(s, params)
	if err != nil {
		return
	}
	f.addAddress(a)
...
//
// NOTE: This extension was ported from github.com/decred/dcrd
func (f *wsClientFilter) existsAddress(a godashutil.Address) bool {
	switch a := a.(type) {
	case *godashutil.AddressPubKeyHash:
...
//
// NOTE: This extension was ported from github.com/decred/dcrd
func (f *wsClientFilter) removeAddress(a godashutil.Address) {
	switch a := a.(type) {
	case *godashutil.AddressPubKeyHash:
...
func (m *wsNotificationManager) subscribedClients(tx *godashutil.Tx,
...
func (m *wsNotificationManager) notifyForNewTx(clients map[chan struct{}]*wsClient, tx *godashutil.Tx) {
	...
	ntfn := btcjson.NewTxAcceptedNtfn(txHashStr, godashutil.Amount(amount).ToBTC())
	marshalledJSON, err := btcjson.MarshalCmd(nil, ntfn)
	if err != nil {
...
// notifications.
func (m *wsNotificationManager) notifyRelevantTxAccepted(tx *godashutil.Tx,
...
func checkAddressValidity(addrs []string, params *chaincfg.Params) error {
	for _, addr := range addrs {
		_, err := godashutil.DecodeAddress(addr, params)
		if err != nil {
...
//
// NOTE: This extension is ported from github.com/decred/dcrd
func rescanBlockFilter(filter *wsClientFilter, block *godashutil.Block, params *chaincfg.Params) []string {
...
func descendantBlock(prevHash *chainhash.Hash, curBlock *godashutil.Block) error {
...
func handleRescan(wsc *wsClient, icmd interface{}) (interface{}, error) {
	...
	for _, addrStr := range cmd.Addresses {
		addr, err := godashutil.DecodeAddress(addrStr, params)
		if err != nil {
			jsonErr := btcjson.RPCError{
				Code: btcjson.ErrRPCInvalidAddressOrKey,
