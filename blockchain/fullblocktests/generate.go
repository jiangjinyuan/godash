...
func makeSpendableOutForTx(tx *wire.MsgTx, txOutIndex uint32) spendableOut {
	return spendableOut{
		...
		amount: godashutil.Amount(tx.TxOut[txOutIndex].Value),
	}
}
...
func payToScriptHashScript(redeemScript []byte) []byte {
	redeemScriptHash := godashutil.Hash160(redeemScript)
...
func calcMerkleRoot(txns []*wire.MsgTx) chainhash.Hash {
	...
	utilTxns := make([]*godashutil.Tx, 0, len(txns))
	for _, tx := range txns {
		utilTxns = append(utilTxns, godashutil.NewTx(tx))
	}
...
func additionalCoinbase(amount godashutil.Amount) func(*wire.MsgBlock) {
...
func additionalSpendFee(fee godashutil.Amount) func(*wire.MsgBlock) {
	return func(b *wire.MsgBlock) {
...
// tests.
func createSpendTx(spend *spendableOut, fee godashutil.Amount) *wire.MsgTx {
...
func createSpendTxForTx(tx *wire.MsgTx, fee godashutil.Amount) *wire.MsgTx {
