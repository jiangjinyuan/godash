...
func addrToKey(addr godashutil.Address) ([addrKeySize]byte, error) {
	...
	case *godashutil.AddressWitnessScriptHash:
		...
		// push to 20-bytes.
		copy(result[1:], godashutil.Hash160(addr.ScriptAddress()))
		return result, nil
...
type AddrIndex struct {
	...
	txnsByAddr      map[[addrKeySize]byte]map[chainhash.Hash]*godashutil.Tx
...
func (idx *AddrIndex) indexUnconfirmedAddresses(pkScript []byte, tx *godashutil.Tx) {
	...
	for _, addr := range addresses {
		...
		if addrIndexEntry == nil {
			addrIndexEntry = make(map[chainhash.Hash]*godashutil.Tx)
...
func NewAddrIndex(db database.DB, chainParams *chaincfg.Params) *AddrIndex {
	return &AddrIndex{
		...
		txnsByAddr:  make(map[[addrKeySize]byte]map[chainhash.Hash]*godashutil.Tx),
